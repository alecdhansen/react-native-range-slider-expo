    ...
    useEffect(() => {
      if (sliderWidth > 0) {
        ...
        if (
          ...
          const offset =
            ((initialValue - min) / step) * stepSize - knobSize / 2;
        ...
        if (valueTextRef != null) {
          const labelValue =
            Math.round(
              ...
              step +
            min;
          valueTextRef.current?.setNativeProps({
            text: formatLabel(labelValue),
          });
        }
        inRangeScaleX.setValue(
...
export default gestureHandlerRootHOC<SliderProps & JSX.IntrinsicAttributes>(
  Slider
);
